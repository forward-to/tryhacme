...
​	空扫描不会设置任何标志；所有六个位标志都被设置为0。

​	使用`-sN`选项选择此扫描。一个没有设置任何标志的TCP数据包在到达开放端口时不会触发任何响应
...
​	FIN 扫描发送一个设置了 FIN 标志的 TCP 数据包。你可以使用 `-sF` 选项选择这种扫描类型。同样，如果 TCP 端口开放，则不会发送响应。再次强调，**Nmap 无法确定端口是开放的还是防火墙阻止了与该 TCP 端口相关的流量。**
...
​	Xmas 扫描的名称来源于圣诞树灯。Xmas 扫描同时设置 FIN、PSH(推送) 和 URG 标志(紧急)。您可以使用选项 `-sX `选择 Xmas 扫描。
